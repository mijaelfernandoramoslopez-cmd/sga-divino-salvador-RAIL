    ...
    public function update(Request $request)
{
    ...
    try {
        ...
        foreach ($request->asistencias as $idstudent => $status) {
            ...
            // Buscamos al estudiante y sus relaciones para notificar el CAMBIO
            $student = Student::with(['user', 'fathers.user'])->find($idstudent);

            if ($student) {
                // Notificación alumno (Modificación)
                if ($student->user) {
                    Notification::create([
                        'iduser' => $student->user->iduser,
                        'title' => 'Actualización de asistencia',
                        'description' => 'Tu asistencia del día ' . $date . ' ha sido modificada a: ' . strtoupper($status),
                        'type' => 'ATTENDANCE',
                        'is_read' => 0,
                        'created_at' => now()
                    ]);
                ...
                // Notificación padres (Modificación)
                foreach ($student->fathers as $father) {
                    if ($father->user) {
                        Notification::create([
                            'iduser' => $father->user->iduser,
                            'title' => 'Actualización de asistencia de ' . $student->full_name,
                            'description' => 'La asistencia de ' . $student->full_name . ' del día ' . $date . ' ha sido modificada a: ' . strtoupper($status),
                            'type' => 'ATTENDANCE',
                            'is_read' => 0,
                            'created_at' => now()
                        ]);
